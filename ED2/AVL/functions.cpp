...
Node* SearchDad(InfoType value, Node* node){
  ...
  if(node->value < value){
    ...
    else SearchDad(value, node->right);	
  }
  ...
  //case o nó a ser inserido seja menor que o atual;
    ...
    else SearchDad(value, node->left);	
  }
}
...
void CreateNode(InfoType value, Node** node){
  ...
  //Cria nós diferentes do primeiro nó da árvore
    ...
    if(dad->value < value) dad->right = new Node(value);
    else if(dad->value > value)  dad->left = new Node(value);
    else cout << "o valor já está inserido na árvore\n";
  }
...
//Faz a busca pelo nodo pai
//Eu sei que jah existe uma funcao parecida
//mas nao confio no codigo
Node* busca(Node *nodo, int valor){
  if(nodo->left != NULL && nodo->left->value == valor) return nodo; 
  else if(nodo->right != NULL && nodo->right->value == valor) return nodo;
  else if(nodo->right != NULL && nodo->value < valor) busca(nodo->right,valor);
  else if(nodo->left != NULL && nodo->value > valor) busca(nodo->left,valor);
}

//Faz exatamente o que o nome diz
Node* buscaMaiorEsquerda(Node *nodo){
  if(nodo->right != NULL && nodo->right->right)
    buscaMaiorEsquerda(nodo->right);
  else return nodo;
}

//Funcao principal da exclusao
Node* excluiNodo(Node *arvore, int valor){
  Node *nodo, *tmp, **filho, *tmp2 = NULL;
  
  if(arvore->value == valor){
    if(arvore->left == NULL && arvore->right == NULL) return NULL;
    filho = &arvore;
  }
  else{
    nodo = busca(arvore,valor);
    if(nodo->left != NULL && nodo->left->value==valor) filho = &nodo->left;
    else filho = &nodo->right;
  }
  if((*filho)->left ==  NULL && (*filho)->right == NULL) *filho = NULL;
  else if((*filho)->left != NULL && (*filho)->right == NULL) (*filho) = (*filho)->left;
  else if((*filho)->left == NULL && (*filho)->right != NULL) (*filho) = (*filho)->right;
  else{
    tmp = buscaMaiorEsquerda((*filho)->left);
    if((*filho)->left == tmp && tmp->right == NULL){
      (*filho)->value = tmp->value;
      (*filho)->left = tmp->left;
      free(tmp);
    }
    else{
      (*filho)->value = tmp->right->value;
      tmp2 = tmp->right;
